...
$(document).ready(function() {// These four lines pick up the variables declared at the very top. If
    ...
    // In other news, vim doesn't indent this correctly because of the unmatched parens
    // in that regex. What a world. 
    var TWO_PARENS = /([)])([(])/;
        // These two used to not recognize the pi replacement, so they didn't work. I fixed it. 
        var LETTER_PAREN = new RegExp("([0-9xy" + PI_REPLACEMENT + "])([(])|([)])([0-9xy" + PI_REPLACEMENT + "])");
        var TWO_LETTERS = new RegExp("([xy" + PI_REPLACEMENT + "])([xy" + PI_REPLACEMENT + "])");
